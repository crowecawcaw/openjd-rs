...
> **Status: developer tool, on a branch — not part of the official package.**
> This harness depends on an out-of-tree Python reference checkout (currently a
> fork/branch), so it is intentionally kept off `main` and out of the shipped
> CI. It is a discovery tool: run it while developing `openjd-expr` to find
> where the Rust port drifts from the Python reference, then encode each finding
> as a plain unit test in the crate (which is what actually ships). If/when the
> `openjd.expr` module lands in the upstream `openjd-model-for-python` mainline,
> this could graduate into official CI without the fork dependency.
...
a crash-only fuzzer and human review both miss. It asks "does any input compute
